...
- Update frame parameter filters for session restoration:
  - Exclude `clone-of` to prevent linking restored frames to stale frame IDs.
  - Exclude `z-group`, `user-size`, and `user-position` by default to defer to the window manager during standard session loads.
  - Include `z-group`, `user-size`, and `user-position` in the geometry inclusion list for explicit layout restoration.
